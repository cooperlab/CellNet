...
int main (int argc, char * argv[])
{
	...
	for(int i=0; i < NUMB_AUGMENTATION_NODE; i++){
		prediction_graph->add_node(new AugmentationNode("augmentation_node" + std::to_string(i), REPEAT_MODE));
	}
	...
	for(int k=0; k < 1; k++){
		...
		for(int i=0; i < NUMB_GRAYSCALE_NODE; i++){
			...
			//prediction_graph->add_edge(new Edge("edge" + std::to_string(n_edges++), "grayscale_node"+ std::to_string(i), "write_pipe_node" + std::to_string(i)));
